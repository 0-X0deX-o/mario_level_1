...
# Called when Mario is in a BIG_TO_FIRE state (i.e. when he obtains a fire flower)
    def changing_to_fire(self):
        ...
        if self.fire_transistion_timer == 0:
            self.fire_transistion_timer = self.current_time
...
# Mario's state and animation when he shrinks from big to small after colliding with an enemy
    def changing_to_small(self):
        ...
        if self.transition_timer == 0:
            ...
            self.hurt_invincible_check()
            self.adjust_rect()
        elif (self.currrent_time - slef.transition_timer) < 590:
            self.image = frames[2]
            self.hurt_invincible_check()
            self.adjust_rect()
           ...
           self.adjuct_rect()
            ...
            self.image = frames[2]
            self.adjust_rect()
            sel.fin_transition_state = False
            self.state = c.WALK
            self.big = False
            self.transition_timer = 0
...
# Makes sure new Rest has the same bottom and left location as previous Rect
	def adjust_rect(self):
		x = self.rect.x
		bottom = self.rect = self.image.get_rect()
		self.rect.x = x
		self.rect.bottom = bottom
		
	def become_small(self):
		self.big = False
		self.right_frames  = self.left_small_normal_frames
		bottom = self.rect.bottom
		left = self.rect.x
		image = self.right_frames[0]
		self.rect = image.get_rect()
...
# State when Mario walks to the castle to end the level
	def walking_to_castle(self):
		...
		elif (self.walking_time - self.walking_timer) > self.calculate_animation_speed():
			if self.frame_index < 3:
				self.frame_index += 1
			else:
				self.frame_index = 1
			self.walking_timer = self.current_time
# State when Mation is falling from the flag pole base
	def falling_at_end_level(self, *args):
...
# Determines if Mario is invincible, Fire Mario or recently hurt
	...
	def check_if_invincible(self):
		if self.invincible:
			...
			elif ((self.current_time - self.invincibility_start_timer) < 12000):
