...
# Extracts image from sprite sheet
    def get_image(self, x, y, width, height):
        ...
        image.blit(self.sprite_sheet, (0, 0) (s, y, width, hieght))
        image.set_colorkey(c.BLACK)
        image = pg.tansform.scale(image,
                (int(rect.width * c.BRICK_SIZE_MULTIPLIER),
                int(rect.height * c.BRICK_SIZE_MULTIPLIER)))
        return image

# Create the group of images based on score received
...
# Updates score movement
    def update(self, score_list, level_info):
            ...
            if score_list:
                    sefl.check_to_delete_floating_scores(score_list, level_info)
